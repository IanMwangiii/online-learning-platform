...
with app.app_context():
    ...
    users = [
        User(name="Alice", username="alice123", email="alice@gmail.com", phone="0123456789", password=generate_password_hash("Password123")),  # Hashing the password
    ...
    enrollments = [
    Enrollment(user_id=users[0].id, course_id=courses[0].id),
    Enrollment(user_id=users[1].id, course_id=courses[1].id),
    Enrollment(user_id=users[2].id, course_id=courses[2].id),
    Enrollment(user_id=users[3].id, course_id=courses[3].id),
    Enrollment(user_id=users[4].id, course_id=courses[4].id),
    Enrollment(user_id=users[5].id, course_id=courses[5].id),
    Enrollment(user_id=users[6].id, course_id=courses[6].id),
    Enrollment(user_id=users[7].id, course_id=courses[7].id),
    Enrollment(user_id=users[8].id, course_id=courses[8].id),
    Enrollment(user_id=users[9].id, course_id=courses[9].id)
]
    ...
    payments = [
        Payment(amount=120000.00, date=date(2024, 1, 10), enrollment_id=enrollments[0].id),
        Payment(amount=150000.00, date=date(2024, 1, 12), enrollment_id=enrollments[1].id),
        Payment(amount=130000.00, date=date(2024, 1, 15), enrollment_id=enrollments[2].id),
        Payment(amount=140000.00, date=date(2024, 1, 18), enrollment_id=enrollments[3].id),
        Payment(amount=160000.00, date=date(2024, 1, 20), enrollment_id=enrollments[4].id),
        Payment(amount=110000.00, date=date(2024, 1, 22), enrollment_id=enrollments[5].id),
        Payment(amount=140000.00, date=date(2024, 1, 25), enrollment_id=enrollments[6].id),
        Payment(amount=150000.00, date=date(2024, 1, 28), enrollment_id=enrollments[7].id),
        Payment(amount=125000.00, date=date(2024, 1, 30), enrollment_id=enrollments[8].id),
        Payment(amount=135000.00, date=date(2024, 2, 1), enrollment_id=enrollments[9].id)
    ]
    ...
    discussions = [
        Discussion(title="Data Analysis Question", content="How do you handle missing data in Python?", user_id=users[0].id, course_id=courses[0].id),
        Discussion(title="Machine Learning Advice", content="What is the best way to optimize a random forest model?", user_id=users[1].id, course_id=courses[1].id),
        Discussion(title="AWS Pricing", content="Can someone explain AWS free tier limitations?", user_id=users[2].id, course_id=courses[2].id),
        Discussion(title="Security Certifications", content="Which certifications are most recognized in cybersecurity?", user_id=users[3].id, course_id=courses[3].id),
        Discussion(title="React State Management", content="What are the best practices for managing state in React apps?", user_id=users[4].id, course_id=courses[4].id),
        Discussion(title="SQL Performance Tips", content="How can I improve the performance of complex SQL queries?", user_id=users[5].id, course_id=courses[5].id),
        Discussion(title="AI Bias", content="What are some methods to mitigate bias in AI models?", user_id=users[6].id, course_id=courses[6].id),
        Discussion(title="IoT Device Security", content="How do you secure IoT devices in a smart home environment?", user_id=users[7].id, course_id=courses[7].id),
        Discussion(title="Agile vs Waterfall", content="When should you choose Agile over Waterfall in software projects?", user_id=users[8].id, course_id=courses[8].id),
        Discussion(title="Blockchain for Supply Chain", content="What are the benefits of using blockchain in supply chain management?", user_id=users[9].id, course_id=courses[9].id)
    ]
